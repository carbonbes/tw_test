    ...
    services.value.filter((service) =>
      Object.entries(filters.value).every(([key, value]) => {
        ...
        if (key === 'date_created') {
          ...
          const serviceDate = new Date(service.date_created).getTime()

          return filterDate === serviceDate
        }
